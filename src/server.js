...
【最重要】法令データベースの取り扱いルール:

**法令に関する質問への回答方法:**
1. 【参考法令データベース】が提供された場合、あなたの事前学習知識は一切使用せず、提供されたデータベースの情報のみを使用してください。
2. 条文番号、章、条文内容は一字一句そのまま引用してください。絶対に変更・推測・補完しないでください。
3. データベースに記載されていない条文については「データベースに情報がありません」と答えてください。
4. あなたの事前学習データに含まれる法令情報は古い可能性があるため、必ずデータベースの情報を優先してください。

【最重要】災害事例データベースの取り扱いルール:

**災害事例に関する質問への回答方法:**
1. 【参考事例データベース】が提供された場合、あなたの事前学習知識は一切使用せず、提供されたデータベースの情報のみを使用してください。
2. タイトル、発生状況、原因、対策、URLは一字一句そのまま使用してください。絶対に変更・推測・補完・省略しないでください。
3. **URLは絶対に自分で作成・推測・変更しないでください。データベースに記載されているURLをそのままコピーしてください。**
4. データベースに記載されていない事例については「データベースに情報がありません」と答えてください。
5. あなたの事前学習データに含まれる事例は使用せず、必ずデータベースの情報を優先してください。
...
3. **事例提示が許される条件（明示的な要求があった場合のみ）**:
   ...
   **事例を提示する際の必須形式:**
   ...
   - 原因: [原因の説明]
   - 対策: [対策の説明]
   - 詳細URL: [データベースに記載されている完全なURLをそのままコピー]

   **重要:**
   - URLは絶対に省略・変更・作成せず、データベースに記載されているURLをそのままコピーすること。
   - URLを自分で推測したり、作成したりすることは絶対に禁止。
...
function formatLawsContext(laws, userMessage = '') {
  ...
  context += '【最重要指示 - 必ず遵守すること】\n';
  context += '1. 以下に記載された条文の内容を一字一句そのまま使用してください。\n';
  context += '2. 条文番号、章、内容を絶対に変更・推測・補完しないでください。\n';
  context += '3. あなたの事前学習知識は使用せず、以下のデータベース情報のみを使用してください。\n';
  context += '4. 以下に記載されていない条文については「データベースに情報がありません」と答えてください。\n';
  context += '5. 以下の第1番目の条文が、ユーザーが質問している条文です。必ずこれを使用してください。\n\n';

  laws.slice(0, 5).forEach((law, index) => {
    if (index === 0) {
      context += `━━━ ユーザーが質問している条文（必ずこれを使用） ━━━\n`;
    }
...
function formatJireiContext(jireiCases, userMessage = '') {
  ...
  if (isAskingForExamples) {
    ...
    jireiCases.slice(0, 3).forEach((jcase, index) => {
      ...
      context += `【URL（必ずこのURLをそのまま記載すること）】\n`;
      context += `${jcase.url || '情報なし'}\n`;
